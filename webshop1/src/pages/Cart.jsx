  ...
  return (
    <div>
      <div className="cart-top">
        ...
        {cart.length === 1 && <div>There is one item in the cart - {calculateItems()} tk </div>} <br />
        {cart.length >= 2 && <div>There is {cart.length} items in the cart - {calculateItems()} tk</div>} <br />
      ...
      <div>
        {cart.map((element, index) =>
         <div className="product" key={index}>
          ...
          <img src="/close.png" alt="" className="button" onClick={() => deleteFromCart(index)} />
          </div>)}
      </div>
    </div>
  )
